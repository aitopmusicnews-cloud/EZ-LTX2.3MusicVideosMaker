...
if (!serverDist.includes('/api/director/chat')) {
  const routes = `app.post("/api/director/chat", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    try {
        ...
        req.log.error({ err: error }, "Director chat failed");
...
console.log("[api build] Compiled Director planning/chat, analyzer-length sections, long-section stitching, social exports, and ten-minute planning timeout for Agnes production.");
