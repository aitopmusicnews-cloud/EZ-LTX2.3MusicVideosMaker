...
export function decodeTaskId(encoded: string): TaskIdPayload {
  try {
    ...
    if (parsed?.source === "modal" && typeof parsed.id === "string") return parsed as TaskIdPayload;
  } catch {
    // Backward compatibility for old projects with an unencoded job id.
  }
...
export async function imageToVideo(req: ImageToVideoRequest, callbackBaseUrl: string): Promise<ModalTask> {
  ...
  try {
    ...
    await writeJobToDisk(jobId, { status: "failed", prompt, error: `Could not reach Modal: ${message}`, createdAt: now, updatedAt: Date.now() });
    throw new Error(`Could not reach the Modal LTX service: ${message}`);
  }
  if (!response.ok) {
    const message = await responseError(response);
    await writeJobToDisk(jobId, { status: "failed", prompt, error: message, createdAt: now, updatedAt: Date.now() });
  ...
  await writeJobToDisk(jobId, { status: "running", prompt, createdAt: now, updatedAt: Date.now(), modalCallId: accepted.call_id });
  return { id: encodeTaskId({ source: "modal", id: jobId }) };
}

/** Generate a Director approval image through the same working Modal media-suite endpoint, with a durable job record. */
export async function generateCharacterFrame(req: TextToImageRequest): Promise<{ id: string }> {
  ...
  try {
    ...
    await writeJobToDisk(jobId, { status: "completed", prompt, image_url: imageUrl, createdAt: now, updatedAt: Date.now() });
    return { id: encodeTaskId({ source: "modal", id: jobId }) };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await writeJobToDisk(jobId, { status: "failed", prompt, error: message, createdAt: now, updatedAt: Date.now() });
...
function absolutePublicUrl(rawUrl: string): string {
  ...
  if (!config.PUBLIC_BASE_URL) throw new Error(`Cannot expose relative media URL without PUBLIC_BASE_URL: ${rawUrl}`);
...
export async function animateLipSync(req: LipSyncRequest): Promise<ModalTask> {
  ...
  try {
    ...
    const response = await fetch(config.MODAL_LIPSYNC_URL, {
      method: "POST", headers: modalHeaders(),
      body: JSON.stringify({ ...videoMedia, ...audioMedia, prompt, reference_strength: referenceStrength, audio_start: req.audioStart ?? 0, audio_end: req.audioEnd, avatar_id: req.avatarId, job_id: jobId, webhook_url: webhookUrl }),
