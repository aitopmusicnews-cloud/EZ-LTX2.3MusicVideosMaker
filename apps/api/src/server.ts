...
const app = Fastify({ logger: { level: "info" }, trustProxy: true, bodyLimit: 100 * 1024 * 1024, ignoreTrailingSlash: true, maxParamLength: 500 });
...
const possibleDirs = [config.WEB_DIST_DIR, resolve(join(__dirname, "..", "..", "web", "dist")), resolve(join(__dirname, "..", "web")), resolve(join(__dirname, "..", "web", "dist")), "apps/web/dist", "../web/dist", "../../apps/web/dist"].filter(Boolean) as string[];
for (const dir of possibleDirs) { const p1 = resolve(dir); if (existsSync(p1) && statSync(p1).isDirectory()) { webDistResolved = p1; break; } const p2 = resolve(join(process.cwd(), "..", "..", dir)); if (existsSync(p2) && statSync(p2).isDirectory()) { webDistResolved = p2; break; } const p3 = resolve(join(process.cwd(), "..", dir)); if (existsSync(p3) && statSync(p3).isDirectory()) { webDistResolved = p3; break; } }
...
app.get("/api/library/images", async (_req, reply) => reply.send({ images: await listImages() }));
app.post("/api/library/images/save", async (req, reply) => { const body = z.object({ id: SafeId, name: z.string().min(1).max(200), url: urlOrPath, source: z.string(), prompt: z.string().nullable(), model: z.string().nullable(), folderId: z.string().nullable().optional() }).parse(req.body); return reply.send(await saveImage(body as any)); });
app.delete("/api/library/images/:id", async (req, reply) => { const params = z.object({ id: SafeId }).parse(req.params); const deleted = await deleteImage(params.id); if (!deleted) return reply.code(404).send({ error: "not found" }); return reply.send({ ok: true }); });
app.get("/api/library/folders", async (_req, reply) => reply.send({ folders: await listFolders() }));
app.post("/api/library/folders/save", async (req, reply) => { const body = z.object({ id: SafeId, name: z.string().min(1).max(200), parentId: z.string().nullable(), type: z.enum(["clips", "images"]) }).parse(req.body); return reply.send(await saveFolder(body as any)); });
app.delete("/api/library/folders/:id", async (req, reply) => { const params = z.object({ id: SafeId }).parse(req.params); const deleted = await deleteFolder(params.id); if (!deleted) return reply.code(404).send({ error: "not found" }); return reply.send({ ok: true }); });
