export function patchDirectorStrictContinuity(source, replaceRequired) {
  ...
  const continuityImport = `${mediaImport}\nimport { buildStrictContinuityInstruction, buildStrictVideoContinuityInstruction, findPriorApprovedContinuityAnchor, findPriorApprovedProjectAnchor } from "../lib/directorContinuityLock.js";`;
  ...
  const newScenePrefix = '{session.plan.shots.map((shot) => { const sceneApproval = session.sceneApprovals[shot.clipId]; const selectedCharacters = selectedCharactersForShot(shot); const selectedCharacterUrls = selectedCharacters.map((identity) => identity.url); const sceneContinuityAnchor = continuityAnchorForShot(shot); const sceneProjectAnchor = projectContinuityAnchorForShot(shot); const sceneReferenceUrls = buildApprovalReferenceImages(sceneApproval?.url, [...selectedCharacterUrls, sceneContinuityAnchor?.url, sceneProjectAnchor?.url]).map((reference) => reference.uri); const sceneIdentityInstruction = buildCharacterIdentityInstruction(selectedCharacters, sceneReferenceUrls); const sceneStrictContinuityInstruction = buildStrictContinuityInstruction({ identities: selectedCharacters, continuityAnchorUrl: sceneContinuityAnchor?.url, projectAnchorUrl: sceneProjectAnchor?.url, referenceUrls: sceneReferenceUrls }); const scenePendingEdit = session.pendingAssetEdits[assetEditKey("scene_image", shot.clipId)]; return';
  ...
  const newShotPrefix = '{session.plan.shots.map((shot) => { const promptWords = words(shot.prompt); const selectedCharacters = selectedCharactersForShot(shot); const selectedCharacterUrls = selectedCharacters.map((identity) => identity.url); const conditioningReady = !shot.requiresCharacter || selectedCharacterUrls.length > 0; const shotApproval = session.shotApprovals[shot.clipId]; const shotContinuityAnchor = continuityAnchorForShot(shot); const shotProjectAnchor = projectContinuityAnchorForShot(shot); const shotReferenceUrls = buildApprovalReferenceImages(shotApproval?.url, [...selectedCharacterUrls, shotContinuityAnchor?.url, shotProjectAnchor?.url]).map((reference) => reference.uri); const shotIdentityInstruction = buildCharacterIdentityInstruction(selectedCharacters, shotReferenceUrls); const shotStrictContinuityInstruction = buildStrictContinuityInstruction({ identities: selectedCharacters, continuityAnchorUrl: shotContinuityAnchor?.url, projectAnchorUrl: shotProjectAnchor?.url, referenceUrls: shotReferenceUrls }); const shotPendingEdit = session.pendingAssetEdits[assetEditKey("shot_image", shot.clipId)]; return';
  ...
  const preparedNew = `    const preparedCharacters = selectedCharactersForShot(shot);\n    const selectedCharacterUrls = preparedCharacters.map((identity) => identity.url);\n    const existingUrl = targetType === "scene_image" ? session.sceneApprovals[clipId]?.url : session.shotApprovals[clipId]?.url;\n    const preparedContinuityAnchor = continuityAnchorForShot(shot);\n    const preparedProjectAnchor = projectContinuityAnchorForShot(shot);\n    const referenceUrls = buildApprovalReferenceImages(existingUrl, [...selectedCharacterUrls, preparedContinuityAnchor?.url, preparedProjectAnchor?.url]).map((reference) => reference.uri);\n    const preparedIdentityInstruction = buildCharacterIdentityInstruction(preparedCharacters, referenceUrls);\n    const preparedStrictContinuityInstruction = buildStrictContinuityInstruction({ identities: preparedCharacters, continuityAnchorUrl: preparedContinuityAnchor?.url, projectAnchorUrl: preparedProjectAnchor?.url, referenceUrls });\n    const preparedPrompt = [pending.prompt, preparedIdentityInstruction, preparedStrictContinuityInstruction].filter(Boolean).join(" ");\n    if (targetType === "scene_image") await generateSceneVisual(clipId, preparedPrompt, referenceUrls);\n    else await generateShotVisual(clipId, preparedPrompt, referenceUrls);`;
  ...
  if (!patched.includes("findPriorApprovedContinuityAnchor")) throw new Error("Director character continuity anchor was not applied.");
  if (!patched.includes("findPriorApprovedProjectAnchor")) throw new Error("Director project continuity anchor was not applied.");
