...
export function patchDirectorLeftRailLauncher(source, replaceRequired) {
  ...
  const analyzerPayload = `clips: clips.map((clip) => ({ id: clip.id, start: clip.start, end: clip.end, sectionLabel: clip.sectionLabel || undefined }))`;
  const visionPayload = `clips: planningClips.map((clip) => ({ id: clip.id, start: clip.start, end: clip.end, sectionLabel: clip.sectionLabel || undefined, userDirection: clip.userDirection || clip.prompt || undefined }))`;
