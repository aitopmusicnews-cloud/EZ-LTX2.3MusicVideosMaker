...
export async function uploadSong(file: File): Promise<{ id: string; audioUrl: string; filename: string }> {
  ...
  fd.append("file", file);
  const uploaded = await jsonOrThrow<{ id: string; url?: string; audioUrl?: string; filename?: string }>(
  ...
  // A newly uploaded song has no analysis yet. Queue analysis immediately so
  // polling always targets a real, newly-created song record.
  await jsonOrThrow<{ status: "pending" }>(
    await fetch(`/api/songs/${encodeURIComponent(uploaded.id)}/analyze`, {
      method: "POST",
